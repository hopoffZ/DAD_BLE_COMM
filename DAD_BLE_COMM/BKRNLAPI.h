...
#include "BTTypes.h"            /* Bluetooth basic type definitions           */
   ...
   /* used with the BTPS_GetTickCount() function to measure elapsed     */
   /* time.                                                             */
#define BTPS_TICK_COUNT_INTERVAL  (1)

   /* Miscellaneous Type definitions that should already be defined, but*/
   /* are necessary.                                                    */
#ifndef NULL
   #define NULL ((void *)0)
#endif
...
typedef void (BTPSAPI *BTPS_MessageOutputCallback_t)(unsigned int Length, char *Message);
...
typedef void (BTPSAPI *BTPS_MailboxDeleteCallback_t)(void *MailboxData);
   ...
   /*          was passed to the BTPS_AddFunctionToScheduler() when the */
   /*          function was added to the scheduler.                     */
   /* * NOTE * Once a Function is added to the Scheduler there is NO way*/
   /*          to remove it.                                            */
...
#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_BTPS_SprintF_t)(char *Buffer, BTPSCONST char *Format, ...);
#endif
...
BTPSAPI_DECLARATION int BTPSAPI BTPS_DumpData(unsigned int DataLength, BTPSCONST unsigned char *DataPtr);
   ...
   /*          largest free fragment and free fragment count will be set*/
   /*          to zero.                                                 */
int BTPSAPI BTPS_QueryMemoryUsage(BTPS_MemoryStatistics_t *MemoryStatistics, Boolean_t AdvancedStatitics);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_BTPS_QueryMemoryUsage_t)(BTPS_MemoryStatistics_t *MemoryStatistics, Boolean_t AdvancedStatitics);
#endif
